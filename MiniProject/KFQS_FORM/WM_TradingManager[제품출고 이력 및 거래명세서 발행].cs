...
namespace KFQS_Form
{
    public partial class WM_TradingManager : DC00_WinForm.BaseMDIChildForm
    {
        ...
        {
            // 거래 명세서는 1:1의 번호 발행이 아닌
            // 상세 내역의 번호를 전부 발행해주어야 하기 때문에 헤더/바디/풋터으로 번호를 발행한다.
            // 헤더는 공통 내역, + 바디는 상세 내역, + 풋터는 추가적인 공통내역
            // 거래 명세서 설정한 데이터들 조회
            ...
            try
            {
                ...
                rtnDtTemp = helper.FillTable("19WM_TradingManager_S3", CommandType.StoredProcedure
                                                            , helper.CreateParameter("PLANTCODE", sPlantcode, DbType.String, ParameterDirection.Input)
                                                            , helper.CreateParameter("TRADINGNO", sTradingNo, DbType.String, ParameterDirection.Input)
                                                            );
